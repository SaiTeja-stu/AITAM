...
import com.cybershield.app.engine.Hashing;
...
public class UpiPaymentPolicy extends AbstractPolicy {
    ...
    public UpiPaymentPolicy(LocalIntelStore intel) {
    ...
    protected List<Signal> doEvaluate(PolicyContext ctx) {
        ...
        if (!vpa.trim().isEmpty() && intel.isReportedVpaHash(Hashing.hmac(vpa))) {
        ...
        boolean looksIndividual = !handle.trim().isEmpty() && !MERCHANT_HANDLES.contains(handle)
